#!/usr/bin/env python3
"""
CourtListener MCP Server - Main Entry Point

A Model Context Protocol server for accessing CourtListener's legal database.
Provides access to court opinions, dockets, clusters, courts, judges, and search capabilities.
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('courtlistener_mcp.log') if os.getenv('MCP_LOG_FILE') else logging.NullHandler()
    ]
)
logger = logging.getLogger(__name__)

# Import the server factory to avoid circular imports
from server_factory import create_courtlistener_server

# Create the MCP server using the factory
mcp = create_courtlistener_server()


def main():
    """
    Main entry point for the CourtListener MCP server.
    
    Supports multiple transport methods and production deployment options.
    """
    ...
    if not os.getenv('COURTLISTENER_API_TOKEN'):
        logger.warning(
            ...
            "Get your token from: https://www.courtlistener.com/profile/tokens/"
        )
    
    # Determine transport method from environment or default to stdio
    transport = os.getenv('MCP_TRANSPORT', 'stdio').lower()
    
    logger.info(f"Starting CourtListener MCP Server with {transport} transport...")
    logger.info("Available tools: opinions, dockets, clusters, courts, search, people, "
                "political-affiliations, aba-ratings, retention-events, sources, education, "
                "citations, opinions-cited, positions")
    
    try:
        if transport == 'stdio':
            # Standard I/O transport (default for Claude Desktop)
            mcp.run()
        elif transport == 'sse':
        ...
        logger.info("CourtListener MCP Server stopped by user")
    except Exception as e:
        logger.error(f"CourtListener MCP Server error: {e}")
        sys.exit(1)
...
# Run with: python test_courtlistener_mcp.py
# """
...
# # Add the project root to Python path for imports (go up one level from tests directory)
# sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# # Import MCP server components
# from courtlistener_server import mcp
...
#         try:
#             # The server should be importable and have tools registered
#             assert hasattr(mcp, 'get_context'), "MCP server not properly initialized"
#             self.logger.info("✅ MCP server initialization successful")
#         except Exception as e:
...
#             # We'll test this by making rapid requests
#         ]
...
#         """Call a tool and measure timing."""
#         start_time = time.time()
#         error = None
#         response = None
...
#                 passed = True
#             else:
#                 # Try to call it as a registered tool
#                 try:
#                     # This would need to be adapted based on how FastMCP exposes tools
#                     # For now, we'll simulate the call
#                     response = f"Simulated call to {tool_name} with {params}"
#                     passed = True
#                 except Exception as e:
#                     error = str(e)
#                     passed = False
...
#             details={'response': str(response)[:500] if response else None}  # Truncate for storage
#         )
...
#         with open('courtlistener_evaluation_report.json', 'w') as f:
#             json.dump(report, f, indent=2, default=str)
        
#         print(f"\n📄 Detailed report saved to: courtlistener_evaluation_report.json")
